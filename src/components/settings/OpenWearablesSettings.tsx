...
export function OpenWearablesSettings() {
  ...
  const l = lang === 'pl' ? {
    ...
    title: 'Open Wearables (selbst gehostet)',
    desc: 'Open Wearables ist eine Open-Source-Plattform, die Daten von 7+ Wearables (Whoop, Garmin, Oura, Polar, Suunto, Strava, Ultrahuman) vereinheitlicht. Selbst über docker compose ausführen und URL + Schlüssel einfügen.',
    baseUrl: 'Server-URL',
    apiKey: 'API-Schlüssel',
    userId: 'Benutzer-ID',
    regenerate: 'Generieren',
    save: 'Speichern',
    show: 'Anzeigen',
    hide: 'Verbergen',
    ...
    statusError: 'Fehler',
    setupHint: 'Führen Sie `docker compose up` in einem Klon von github.com/the-momentum/open-wearables aus, dann kopieren Sie den API-Schlüssel aus dem Admin-Panel unter localhost:3000.',
    guideTitle: 'Schritt für Schritt — wie richte ich das ein?',
  } : {
    title: 'Open Wearables (self-hosted)',
  ...
  const guideSteps = lang === 'pl' ? [
    ...
    <>Klonen Sie das Open Wearables Repository:<br /><code className="text-[10px] bg-bg-card px-1.5 py-0.5 rounded">git clone https://github.com/the-momentum/open-wearables.git</code></>,
    <>Wechseln Sie in den Ordner und starten Sie Docker (Docker Desktop muss installiert sein):<br /><code className="text-[10px] bg-bg-card px-1.5 py-0.5 rounded">cd open-wearables &amp;&amp; docker compose up -d</code><br /><span className="text-text-tertiary">[Screenshot: Terminal nach docker compose up]</span></>,
    <>Öffnen Sie das Admin-Panel im Browser: <a href="http://localhost:3000" target="_blank" rel="noopener" className="text-lavender-700 underline">http://localhost:3000</a> und melden Sie sich mit den Standardanmeldedaten an: <strong>admin@admin.com</strong> / <strong>your-secure-password</strong> (ändern Sie diese vor jeder echten Verwendung).<br /><span className="text-text-tertiary">[Screenshot: Open Wearables-Login]</span></>,
    <>Im Admin-Panel gehen Sie zu <strong>Settings → API Keys</strong> und klicken auf <strong>Generate</strong>. Kopieren Sie den generierten Schlüssel.<br /><span className="text-text-tertiary">[Screenshot: API Keys-Panel]</span></>,
    <>Kommen Sie hierher zurück. Geben Sie ein: <strong>Server-URL</strong> = <code className="text-[10px] bg-bg-card px-1.5 py-0.5 rounded">http://localhost:8000</code>, <strong>API-Schlüssel</strong> = kopierten einfügen, <strong>User ID</strong> = klicken Sie „Generieren" oder leer lassen (auto). Klicken Sie auf <strong>Speichern</strong>.</>,
    <>Nach dem Speichern erscheint eine „Verwalten"-Schaltfläche auf dem <strong>Daten</strong>-Tab — klicken Sie darauf, wählen Sie ein Gerät (z. B. Oura, Whoop), melden Sie sich beim Anbieter an, fertig.</>,
    <>Vollständige Textanleitung in der Open Wearables-Dokumentation: <a href="https://openwearables.io/docs/quickstart" target="_blank" rel="noopener" className="text-lavender-700 underline">openwearables.io/docs/quickstart</a></>,
  ] : [
